...
export class RadiusController {
    ...
    async adicionarUsuario(@Body() body: RadiusTestDto) {
        return {
            success: await this.radiusService.adicionarUsuario(
                body.username, 
                body.password, 
                body.ip,
            ),
    ...
    async testarAutenticacao(@Body() body: RadiusTestDto) {
        if (!body.password) {
            return { success: false, message: 'Senha é obrigatória para autenticação.' };
        }
        return {
