...
import {
  FiltroRelatoriosDto,
  TipoRelatorio,
} from './dto/filtro-relatorios.dto';
...
export class RelatoriosService {
  ...
  private async relatorioResumoMensal(filtro: FiltroRelatoriosDto) {
    ...
    return {
      ...
      metricas: {
        receitas: Number(receitas.total) || 0,
      ...
      resumo: {
        receitaLiquida:
          (parseFloat(receitas.total) || 0) -
          (parseFloat(pendentes.total) || 0),
  ...
  private async relatorioClientesInadimplentes(filtro: FiltroRelatoriosDto) {
    ...
    const clientesInadimplentes = await this.clienteRepository
      ...
      .leftJoinAndSelect('cliente.planos', 'planos')
      .where('fatura.paga = :paga', { paga: false })
      .andWhere('fatura.dataVencimento < :hoje', { hoje })
      .andWhere('cliente.ativo = :ativo', { ativo: true })
      .getMany();
    ...
    const clientesDetalhados = clientesInadimplentes.map((cliente) => {
      const faturasPendentes = cliente.faturas.filter(
        (fatura) => !fatura.paga && fatura.dataVencimento < hoje,
      );
      ...
      const diasAtraso =
        faturasPendentes.length > 0
          ? Math.floor(
              (hoje.getTime() - faturasPendentes[0].dataVencimento.getTime()) /
                (1000 * 60 * 60 * 24),
            )
          : 0;
    ...
    return {
      ...
      totalClientes: clientesDetalhados.length,
      totalDevido: clientesDetalhados.reduce(
        (acc, cliente) => acc + cliente.totalDevido,
        0,
      ),
      ...
      metricas: {
        maiorDivida:
          clientesDetalhados.length > 0
            ? Math.max(...clientesDetalhados.map((c) => c.totalDevido))
            : 0,
        mediaDivida:
          clientesDetalhados.length > 0
            ? clientesDetalhados.reduce((acc, c) => acc + c.totalDevido, 0) /
              clientesDetalhados.length
            : 0,
      },
    };
  }
  ...
  private async relatorioReceitasPlanos(filtro: FiltroRelatoriosDto) {
    ...
    const receitasPorPlano = await this.faturaRepository
      ...
      .leftJoin('cliente.faturas', 'fatura')
      .leftJoin('cliente.planos', 'plano')
      .addSelect('COUNT(fatura.id)', 'totalFaturas')
      .addSelect('COUNT(DISTINCT cliente.id)', 'totalClientes')
      .addSelect(
        'SUM(CASE WHEN fatura.paga = true THEN fatura.valor ELSE 0 END)',
        'receita',
      )
    ...
    return {
      ...
      planos: receitasPorPlano.map((plano) => ({
        ...
        participacao:
          totalReceita > 0
            ? (parseFloat(plano.receita || 0) / totalReceita) * 100
            : 0,
      })),
      total: {
        reiceitaTotal: totalReceita,
        clientesTotal: receitasPorPlano.reduce(
          (acc, plano) => acc + parseInt(plano.totalClientes),
          0,
        ),
      },
    };
  ...
  private async relatorioHistoricoPagamentos(filtro: FiltroRelatoriosDto) {
    ...
    return {
      ...
      resumo: {
        ...
        valorTotal:
          pagamentos.reduce(
            (acc, p) => acc + parseFloat(p.valor.toString()),
            0,
          ) / pagamentos.length || 1,
      },
  ...
  private processarDatas(filtro: FiltroRelatoriosDto): {
    dataInicio: Date;
    dataFim: Date;
  } {
    let dataInicio: Date;
  ...
  async dashboard() {
    ...
    const receitaMes = await this.faturaRepository.find({
      where: {
        dataPagamento: Between(inicioMes, fim),
      },
    });

    const total = receitaMes.reduce((acc, item) => acc + item.valor, 0);
    const clientesAtivos = await this.clienteRepository.count({
      where: { ativo: true },
    });
