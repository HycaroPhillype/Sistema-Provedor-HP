...
describe('ClientesController (e2e)', () => {
    ...
    beforeAll(() => {
        ...
        clientesRepo = moduleFixture.get<Repository<Cliente>>(getRepositoryToken(Cliente));
    });
    ...
    it('/clientes/:id (DELETE) - deve retornar 404 se o cliente não existir', async () => {
        await request(app.getHttpServer())
            ...
            .expect(404)
            .then((response) => {
                expect(response.body.message).toBe('Cliente com ID 9999 não encontrado');
            }
        );
    });
    ...
    it('deve chamar o serviço RADIUS ao remover cliente', async () => {
        ...
        console.log(`[VERIFICAÇÃO] O usuário ${cliente.login} foi removido do RADIUS.`);
